...
echo "    - Samba/Time Machine (macOS Time Machine target - modules/timemachine.nix)"
echo "      Primary instance runs on nixos2. Enable here for failover:"
echo "        1. Edit /etc/nixos/modules/timemachine.nix"
echo "        2. Set: services.samba.enable = true;"
echo "                services.samba-wsdd.enable = true;"
echo "        3. Add firewall ports to /etc/nixos/modules/networking.nix:"
echo "           allowedTCPPorts: 139, 445"
echo "           allowedUDPPorts: 137, 138, 5353"
echo "        4. Rebuild: sudo nixos-rebuild switch"
echo "        5. Set Samba password: sudo smbpasswd -a tmuser"
echo ""
...
echo ""
echo "Collabora status by branch:"
echo "  - MAIN branch (production server): ENABLED"
echo "  - VM branch / nixos2 (standby/failover): DISABLED"
echo ""
...
echo "     collabora.home → YOUR_IP (e.g., 192.168.1.154)"
echo ""
echo "  2. Open Nextcloud admin settings:"
echo "     http://cloud.home → Top-right user menu → Administration settings"
echo ""
echo "  3. Navigate to Office settings:"
echo "     Left sidebar → Administration → Office"
echo ""
...
echo ""
echo "═══════════════════════════════════════════════════════════════════════════"
echo "🖥️  OPTIONAL: VNC Remote Desktop Access"
echo "═══════════════════════════════════════════════════════════════════════════"
echo ""
echo "If x11vnc + noVNC are enabled in modules/services.nix, a password file"
echo "is required before x11vnc will accept connections."
echo ""
echo "REQUIRED (if VNC is enabled) - Create the VNC password:"
